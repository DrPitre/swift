...
// Diagnostics testing
// RUN: %target-swift-frontend(mock-sdk: -enable-source-import -sdk %t/sdk -I %t/sdk/swift-modules) -typecheck -verify -swift-version 5 -enable-experimental-feature MacrosOnImports -load-plugin-library %t/%target-library-name(MacroDefinition) -module-name ModuleUser %s -I %t

// Emit IR to ensure that we are handling the created body end-to-end.
// RUN: %target-swift-frontend(mock-sdk: -enable-source-import -sdk %t/sdk -I %t/sdk/swift-modules) -emit-ir -swift-version 5 -g -enable-experimental-feature MacrosOnImports -load-plugin-library %t/%target-library-name(MacroDefinition) -module-name ModuleUser %s -I %t -validate-tbd-against-ir=none | %FileCheck %s
